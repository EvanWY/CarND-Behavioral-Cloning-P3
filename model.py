    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                ...
                images.append(cv2.imread('records/IMG/'+batch_sample[0].split('/')[-1]))
                angles.append(frame_angle)
                images.append(cv2.imread('records/IMG/'+batch_sample[1].split('/')[-1]))
                angles.append(frame_angle + 0.05)
                images.append(cv2.imread('records/IMG/'+batch_sample[2].split('/')[-1]))
                angles.append(frame_angle - 0.05)
